...
if($count !=0){
    while($cart=mysqli_fetch_array($a)){
        ...
        $product_info="UPDATE `product` SET`Renaining_Quantity`=Renaining-$quantity WHERE Product_Id='$product_id'";
    }
    $delete="DELETE FROM `cart` WHERE user_id='$user_id'";
    mysqli_query($con,$delete);
    $_SESSION['cart_order']="Cart order Added to Cart";
}else{
    $_SESSION['cart_order']="Please add product to cart";
}
?>
<script>
