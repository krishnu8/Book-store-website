...
session_start();
    ...
    if (isset($_POST['regi'])) {
        ...
        if (mysqli_query($con, $q)) {
            ...
            $mail->Username   = "rdas570@rku.ac.in";  // GMAIL username(from)
            $mail->Password   = "RAMESH DAS";            // GMAIL password(from)
            $mail->SetFrom('rdas570@rku.ac.in', 'Ecommerce Website'); //from
            ...
            if (!$mail->Send()) {
                $_SESSION['reg_msg_err'] = "Error. Error in sending activation link. Please try again.";
    ?>
                <script>
