<?php
    ...
    if (isset($_POST['signup'])) {
        ...
        // count erros
        if (count($errors) === 0) {
        //     $insertQuery = "INSERT INTO registration (firstName,lastName,email,password,gender,code,status)
    ...
    if (isset($_POST['login'])) {
        ...
        $emailQuery = "SELECT * FROM registration WHERE role = 'Admin'";
        $email_check = mysqli_query($con, $emailQuery);

        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM registration WHERE email = '$email' AND password = '$password'";
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM registration WHERE role = 'Admin'";
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $code = rand(999999, 111111);
                $updateQuery = "UPDATE registration SET User_id = $code WHERE role = 'Admin'";
                $updateResult = mysqli_query($con, $updateQuery);
                if ($updateResult) {
                    $subject = 'Email Verification Code';
                    $message = "our verification code is $code";
                    $sender = 'From: rdas570@rku.ac.in.com';
...
if(isset($_POST['verifyEmail'])){
    ...
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE registration SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePassword = "UPDATE registration SET password = '$password' WHERE role = 'Admin'";
